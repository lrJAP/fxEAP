...
本项目中相关Markdown文件将不再更新，只更新代码，如果需要查看最新的文档，请访问[www.lrJAP.com](http://www.lrJAP.com)

## fxEAP JavaFX开发手册

[fxEAP JavaFX开发手册](doc/javafx/README.md)，本文档描述fxEAP对JavaFX组件的扩展与增加。

## fxEAP模式化开发简介

[fxEAP模式化开发](doc/ui/README.md)，这是fxEAP中**非常重要**的一份文档，描述如何基于元数据结合格式化开发机制，实现功能的快速开发。
...
| [元数据设计](doc/metadata/README.md)                       | 基于PowerDesigner，生成单一实体、树形结构的元数据信息，包括实体信息、实体属性信息、查询条件相关的前后端代码。<br />元数据是fxEAP中最基础的公共数据，是后续聚合对象设计、多对多对象设计、单据及功能菜单开发的基础。 |
...
| 模块名称                                                     | 说明                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| [eap-platform/eap-platform-base](eap-platform/eap-platform-base/README.md) | 提供ValueObject、FX ValueObject、Reference参照系统、JavaFX扩展等 |
| [eap-platform/eap-platform-base-private](eap-platform/eap-platform-base-private/README.md) | 提供BusinessObject体系等                                     |
| [eap-platform/eap-platform-javafx](eap-platform/eap-platform-javafx/README.md) | 扩展并增强JavaFX界面组件                                     |
...
fxEAP用于验证上述愿景的可行性。**在基于平台+模式化开发体系的支撑下，开发人员应该把主要精力放在实现业务逻辑上**。软件公司或是编码人员的本质工作是什么？我们认为是**解决客户的问题，满足客户的需求**。
...
- 持久化方面，基于MyBatis，结合fxEAP的Value Object(VO)体系，实现高效、易用的O-R Mapping机制，减少与数据操作相关的代码、配置文件以及SQL等配置信息。fxEAP持久化的目标，是以MyBatis(直接、少量SQL语句)的**性能**，达到Hibernate(VO映射)的**易用性**。包括便于前台界面展现、中间件业务逻辑处理及持久化、其它方面数据交互等方面的易用性。这是fxEAP基础价值的重要体现。
...
### 下载并配置JavaFX SDK
   ...
   - 配置IDEA
     ...
     参见：[fxEAP JavaFX环境配置](doc/demo/JavaFX_Env.md)

### 下载代码到本地

[fxEAP GitHub](https://github.com/lrJAP/EAP)

```java
git clone https://github.com/lrJAP/fxEAP.git
```

[fxEAP Gitee](https://gitee.com/lrJAP/fxEAP)
...
### 创建并初始化数据库
   ...
   参考：[fxEAP演示环境数据库初始化](doc/demo/DataBase.md)
...
### 把工程导入IntelliJ IDEA
   ...
   参考：[fxEAP演示环境IDEA初始化](doc/demo/IDEA.md)
...
- 下载[fxEAP演示环境的WebClient客户端](TODO)
